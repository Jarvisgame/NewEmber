...
pe_meta_dir = config['svm_classifier_jsonl']['input_dir']
output_dir  = config['svm_classifier_jsonl']['output_dir']
Utils.check_directory_exists(output_dir)
model_path  = os.path.join(output_dir, 'svm_model_jsonl.pkl')

# 评估/随机种子/模型超参
test_size     = config['svm_classifier_jsonl']['test_size']
random_state  = config['svm_classifier_jsonl']['random_state']
C             = config['svm_classifier_jsonl'].get('C', 1.0)
max_iter      = config['svm_classifier_jsonl'].get('max_iter', 2000)
class_weight  = config['svm_classifier_jsonl'].get('class_weight', 'balanced')
